...
void il2cpp::init()
{
	...
	using il2cpp_image_get_class_count = size_t(*)(uintptr_t image);
	methods::image_get_class_count = LI_FIND_DEF(il2cpp_image_get_class_count);

	using il2cpp_image_get_class = size_t(*)(uintptr_t image, size_t index);
	methods::image_get_class = LI_FIND_DEF(il2cpp_image_get_class);

	using il2cpp_class_get_namespace = const char* (*)(uintptr_t);
	methods::class_get_namespace = LI_FIND_DEF(il2cpp_class_get_namespace);

	using il2cpp_class_get_name = const char* (*)(uintptr_t);
	methods::class_get_name = LI_FIND_DEF(il2cpp_class_get_name);
...
uintptr_t il2cpp::init_class(const char* name, const char* name_space)
{
	...
	for (int i = 0; i < nrofassemblies; i++)
	{
		...
		size_t class_count = methods::image_get_class_count(img);
		for (size_t j = 0; j < class_count; j++)
		{
			uintptr_t kl = methods::image_get_class(img, j);
			if (!kl)
			{
				continue;
			}

			const char* current_name = methods::class_get_name(kl);
			if (strcmp(current_name, name) != 0)
			{
				continue;
			}

			const char* current_namespace = methods::class_get_namespace(kl);
			if (strcmp(current_namespace, name_space) != 0)
			{
				continue;
			}
...
uintptr_t il2cpp::hook(void* our_func, std::string kl, std::string name, int arg, const char* name_space)
{
	...
	*reinterpret_cast<void**>(il2cpp_method) = our_func;
	...
	if (written == our_func)
	{
		...
		std::cout << "[Debug] Failed to Hook " << kl << std::endl;
	}
